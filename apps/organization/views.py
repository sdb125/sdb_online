...
class AddFavView(View):
    ...
    def post(self, request):
        ...
        if not request.user.is_authenticated:
            ...
            return HttpResponse('{"status":"fail", "msg1":"用户未登录"}', content_type='application/json')

        exist_record = UserFavorite.objects.filter(user=request.user, fav_id=int(fav_id), fav_type=int(fav_type))
        if exist_record:
            # 如果记录已经存在，表示用户取消收藏
            exist_record.delete()
            return HttpResponse('{"status":"fail","msg1":"已取消收藏" ,"msg":"收藏"}', content_type='application/json')
        else:
